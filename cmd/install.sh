...
${B}Env vars:${R}
  ...
  ${YLW}TN_LANG${R}=en|zh           $(t "Set language" "设置语言")
EOF
}
...
init_temp_dir() { TEMP_DIR="$(mktemp -d 2>/dev/null || mktemp -d -t tn)"; }
...
fetch_latest_version() {
    ...
    local latest_asset="${BINARY_NAME}-linux-amd64"
